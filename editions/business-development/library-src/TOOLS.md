...
<!-- The `autoresearch` skill doc is deliberately not shipped in this seat (see the assembly report). The commands below still work; this seat's improvement loop is the calibration digest and message-class graduation instead. -->
| Command | What it does |
|---|---|
...
### Worker Sessions
<!-- The `worker-agents` skill doc is deliberately not shipped in this seat (see the assembly report). The commands below still work if the install ever needs them. -->
| Command | What it does |
|---|---|
...
Command surfaces above are framework-wide. The BD seat's own workflows live here:

| Area | Skill |
|---|---|
| The board: schema, tabs, write rules | `pipeline-board` |
| Stage entry/exit, required fields, max days, archive rules | `stage-gates` |
| The morning run and the alert register | `daily-pipeline-run` |
| New leads, speed-to-lead, redirects, early declines | `lead-intake` |
| The discovery call and the pre-handoff prevention checklist | `discovery-call` |
| The method: stage flow, tone, question sets | `question-led-selling` |
| Objections, and the competitor-price conversation | `objection-handling` |
| Rate presentation, package presentation, the close | `pricing-presentation` |
| Appointments: decision-makers, ownership verification, pre-appointment touches | `listing-appointment` |
| Agreement send, signature chase, execution checks, handoff package, referral flag | `pma-and-handoff` |
| **The Never-Promise List** | `never-promise-list` |
| Fee deviation and contract modification gates | `fee-and-contract-gates` |
| Problem property, takeover, and walk-away gates | `property-and-owner-gates` |
| Protected-class matters — route, never answer | `fair-housing-guard` |
| How an escalation is logged and routed | `escalation-log` |
| Follow-up cadence, nurture, cold recovery | `followup-and-nurture` |
| Conversion metrics, weekly review, monthly report | `pipeline-metrics-and-review` |
| Prospect data, retention, opt-outs, source compliance | `prospect-data-handling` |
| Staging and releasing owner-facing messages | `draft-release-gate` |
| The first week | `shadow-mode-calibration` |
