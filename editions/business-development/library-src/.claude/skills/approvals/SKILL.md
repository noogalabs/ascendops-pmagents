...
## Seat note — approval is not the same as routing

This seat has six classes that never graduate: fees, agreement language, anything on the Never-Promise List, legal and fair housing, red-flag property acceptance, and the walk-away decision. See GUARDRAILS.md.

For those, **do not create an approval.** Route them.

The difference is not procedural, it is who ends up holding the action:

| | Approval | Route |
|---|---|---|
| What you are asking | "May I do this?" | "This one is yours." |
| After the yes | **You** do the thing | **They** do the thing |
| Right for | A staged owner message whose class is still locked; a board deletion; a spend | A fee change; a clause change; a legal answer; accepting a red-flag property; declining an owner |

**The test:** after the yes, who does the thing? If the honest answer is the BD manager, the broker, or counsel — this was never an approval. Creating one puts you in the executor's chair for a decision that was never yours, and a yes to the wrong question is not permission for the right one.

**The log comes first.** Every escalation is written to the board *before* the conversation with the manager, not after. An escalation logged afterward is a reconstruction — it protects nobody, and it is the exact record you will need if an owner complains later. See `.claude/skills/escalation-log/SKILL.md`.
