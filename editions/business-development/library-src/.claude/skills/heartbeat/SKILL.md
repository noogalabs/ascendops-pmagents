...
if [[ "$HEARTBEAT_NOOP" != "true" ]]; then
  ...
  cortextos bus list-tasks --status pending --format json | jq '[.[] | select(.assigned_to == "human" or .project == "human-tasks")]'
...
- If pending >4h AND the structured config says it is inside the maintenance-owned day-mode window AND no re-ping has been sent yet for this approval → send ONE re-ping:
  ```bash
...
- Outside the configured window, or while either day-mode pointer is absent: skip re-pings and defer to the next configured day-mode heartbeat.
...
**Never claim a status you haven't verified.** Crons are daemon-managed and survive restarts — confirm with `cortextos bus list-crons` before saying "crons running."
...
## Seat note — the board alerts are this seat's real queue

On this seat, an empty task list does not mean idle. The alert register on the pipeline board is the queue that never runs dry, and it outranks a quiet inbox. Every heartbeat, after the approvals sweep, run the alert pass in `HEARTBEAT.md` Step 3b.

Two seat rules that hold on every cycle:

- **Nothing owner-facing leaves on a heartbeat.** Not a nudge, not a confirmation, not a "quick check-in". Heartbeat work is board work, research, and staged drafts. Release is a human's, on their clock.
- **An alert that survives two consecutive cycles is itself the finding.** Surface it to the BD manager rather than working it a third time quietly.
