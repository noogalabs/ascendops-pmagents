...
if [[ "$HEARTBEAT_NOOP" != "true" ]]; then
  ...
  #   - If night mode (after {{day_mode_end}}): defer reminders to the next day's first heartbeat
...
- If pending >4h AND it's day mode ({{day_mode_start}}-{{day_mode_end}} local) AND no re-ping has been sent yet for this approval → send ONE re-ping:
  ```bash
...
- Night mode (after {{day_mode_end}}): skip re-pings, defer to next day's first heartbeat.
