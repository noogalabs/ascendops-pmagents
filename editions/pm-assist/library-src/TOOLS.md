...
All cortextOS commands: `cortextos bus <command>` (the `cortextos` binary is identical). Full docs in skill files — load the relevant skill when you need details on a workflow.
...
> The `autoresearch` skill is not carried by this seat — the seat's improvement loop is the calibration digest (`shadow-mode-calibration`) and message-class graduation (`draft-release-gate`). These commands remain available.
| Command | What it does |
|---|---|
...
### Seat skills — the operating board and its clocks
| Skill | What it covers |
|---|---|
| `pm-operating-board` | Workbook shape, the nine tabs, pull rules, source-and-pull-time convention |
| `daily-pulse` | The morning build and the surface message |
| `monday-board` | The week ahead + the Follow-Through Log sweep |
| `month-end-pack` | Close, compliance rows, KPI roll-up |
| `approval-queue` | Spend requests, aging, the owner non-response ladder |
| `escalation-triage` | Front gate for every exception; where gated matters get classified |
| `owner-snapshot` | Per-owner tag, channel, reserve, open items |
| `owner-report-pack` | Monthly pack assembly and the release gate |
| `alert-rules` | The threshold register across all five domains |
| `decision-log` | The write-it-down step |
| `kpi-scorecard` | Occupancy, close rate, renewal rate, retention, expiry concentration |
| `renewal-pipeline` | Look-ahead, CMA request, owner decision window, tenant follow-ups |
| `delinquency-clock` | Late notice, alert day, portfolio target |
| `compliance-calendar` | State filings, notice clocks, retention, inspection cadence |
| `broker-escalation` | The broker-only decision classes and the same-day channel |
| `fair-housing-guard` | Protected-class matters: route, never answer |
| `draft-release-gate` | Message classes, staging, and graduation |
| `shadow-mode-calibration` | The first-week digest and how shadow mode ends |
