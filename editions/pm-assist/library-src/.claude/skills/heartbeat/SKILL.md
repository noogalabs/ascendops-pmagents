...
if [[ "$HEARTBEAT_NOOP" != "true" ]]; then
  ...
  cortextos bus list-tasks --status pending --format json | jq '[.[] | select(.assigned_to == "human" or .assigned_to == "property_manager" or .project == "human-tasks")]'
  ...
  #   (c) GRANTED: a standing rule in SOUL.md gives this agent org-wide queue ownership.
  # For each task selected above that PASSES the lane gate (selection is by
  # human/property_manager assignee OR "human-tasks" project):
  ...
  #   - If night mode (outside {{day_mode_start}}-{{day_mode_end}} {{timezone}}): defer reminders to the next morning build
  # For an out-of-lane task: do not contact {{property_manager_name}}; note it on
  # Escalation Triage with its real owner. Project membership is discovery, not authority.
...
- If pending >4h AND it's day mode ({{day_mode_start}}-{{day_mode_end}} {{timezone}}) AND no re-ping has been sent yet for this approval → send ONE re-ping:
  ```bash
...
- Night mode (outside {{day_mode_start}}-{{day_mode_end}} {{timezone}}): skip re-pings, defer to next day's first heartbeat.
