...
# Notify the Property Manager so it surfaces in the Daily Pulse.
# Where an orchestrator agent is installed alongside this seat, also notify it.
cortextos bus send-telegram $CTX_TELEGRAM_CHAT_ID "Human task created: [HUMAN] <title> — needed before I can proceed with <your task>"
[ -n "$CTX_ORCHESTRATOR_AGENT" ] && cortextos bus send-message $CTX_ORCHESTRATOR_AGENT normal "Human task created: [HUMAN] <title>"
```
