...
officecli add $DOC /body --type paragraph \
  --prop text="Example Property Operations Monthly Report — $(date +%B\ %Y)" \
  --prop style=Heading1
...
officecli add $DECK / --type slide \
  --prop title="Welcome to Example Property Operations" \
  --prop background=1A1A2E

# Content slide
officecli add $DECK / --type slide --prop title="Your AI Agent Team"
officecli add $DECK '/slide[2]' --type shape \
  --prop text="• Agent One — Orchestrator\n• Agent Two — PM Specialist\n• Agent Three — Analyst\n• Agent Four — Dev Agent" \
