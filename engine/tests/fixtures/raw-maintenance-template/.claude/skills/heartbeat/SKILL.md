...
if [[ "$HEARTBEAT_NOOP" != "true" ]]; then
  ...
  #   (c) GRANTED: AGENTS.md gives this agent org-wide queue ownership.
  # For each task selected above that PASSES the lane gate (selection is by
  # human assignee OR "human-tasks" project):
  ...
  #   - If night mode (after 19:30 in the agent configured local timezone ($CTX_TIMEZONE)): defer reminders to next morning-review
  # For an out-of-lane task: do not contact the owner; escalate internally to the
...
- If pending >4h AND it's day mode (07:30-19:30 in the agent configured local timezone ($CTX_TIMEZONE)) AND no re-ping has been sent yet for this approval → send ONE re-ping:
  ```bash
...
- Night mode (after 19:30 in the agent configured local timezone ($CTX_TIMEZONE)): skip re-pings, defer to next day's first heartbeat.
