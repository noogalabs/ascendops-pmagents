...
Classify the urgent-looking situation passed in `$ARGUMENTS` (a meld ID or short description, e.g. `door kicked in, secured, on-call tech coming 8am` or `MELD123 AC out`).

**Why this is a forked Sonnet skill:** emergency-vs-contained is a repeating judgment call with a fixed decision tree, not open-ended reasoning — Sonnet is the right tier. It is `context: fork` so the classification runs in its own window and the main session stays lean; it also keeps the urgent-escalation decision OFF the reflex path, forcing the stand-down check to fire before anything reaches the property manager. Opus is not needed; disciplined classification is.
...
2. **Confirm full body AND current meld state before any urgent escalation.** Never fire a manager-facing urgent off a truncated fast-checker snippet plus a partial/first parse. (Real incident, MELD123 false urgent: claimed "unassigned, no vendor, baby in the house" — ALL false; the meld was COMPLETED, the tech was returning, and the baby detail was hallucinated. Delayed shell-output flushing was the trap.) Verify before confirmed, in the escalation path especially.
...
/emergency-classify MELD123 tenant says AC out
```
