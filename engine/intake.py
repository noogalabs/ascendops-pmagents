...
US_STATE_NAME = re.compile(
    rf"\b(?:{'|'.join(re.escape(name) for name in US_STATE_NAMES)})\b",
    re.I,
)
...
def _is_prose_form_jurisdiction_clock(line: str) -> bool:
    ...
    return has_jurisdiction_subject and has_integer_duration
...
def _validate_accounting_scope(answers: dict[str, str], failures: list[tuple[str, str]]):
    ...
    scoped = [line for line in lines if (
        ...
        or _is_prose_form_jurisdiction_clock(line)
