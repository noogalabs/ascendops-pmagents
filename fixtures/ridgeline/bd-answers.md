...
Owner speed-to-lead window: 30 minutes

This value has no question, no placeholder, and no config key in the draft tree today, and the
seat references "the speed-to-lead window" in 13 places across 10 files - including a Critical
alert class in the heartbeat cron. It is recorded here so the gap is visible rather than implied.
See finding F-BD-1 in the mapping table.

Pointer-seeded values, NOT cover-sheet fields: the outreach window
({{day_mode_start}} / {{day_mode_end}}) is seeded from the org `context.json` and owned by the
maintenance seat's B8 answer. It is recorded as a pointer so the member confirms it, and it is
deliberately not minted as a further cover-sheet field (leasing X2 precedent).
